...
if [[ -d "$VM_REPO" ]]; then
  ...
  [[ -x "$VM_REPO/scripts/sync-agent-files.sh" ]] \
    || die "$VM_REPO/scripts/sync-agent-files.sh is missing or not executable"
  "$VM_REPO/scripts/sync-agent-files.sh" "$REPO"
  "$VM_REPO/scripts/sync-agent-files.sh" install
